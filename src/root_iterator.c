...
struct context {
  root_iterator_t iterator;
  struct region* onlyIn;
  struct heap* heap;
};

static bool iterateRoot(struct root* root, struct context* ctx) {
  for (int i = 0; i < root->size; i++) {
    ...
    struct object_info* obj = heap_get_object_info(ctx->heap, ref);
    if (!ctx->iterator(&root->entries[i], obj))
      return false;
  }
  return true;
}

static bool iterateThread(struct thread* thread, struct context* ctx) {
  // Iterate each frame
  for (int i = 0; i < thread->framePointer; i++)
    if (thread->frames[i].isValid)
      if (!iterateRoot(thread->frames[i].root, ctx))
        return false; 
  return true;
}
...
bool root_iterator_run(struct heap* heap, struct region* onlyIn, root_iterator_t iterator) {
  ...
  for (int i = 0; i < heap->threadsListSize; i++) {
    if (heap->threads[i])
      if (!iterateThread(heap->threads[i], &ctx))
        return false;
  }
