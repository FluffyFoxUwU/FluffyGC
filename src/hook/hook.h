...
#define HOOK_FUNCTION(spec, ret, name, ...) \
  ...
  spec ____HOOK_DECLARE(void, name, [[maybe_unused]] NULLABLE(ret*), returnLocation, [[maybe_unused]] NONNULLABLE(struct hook_call_info*), ci __VA_OPT__(,) __VA_ARGS__)

#define HOOK_FUNCTION_VOID(spec, name, ...) \
  typedef void (*____HOOK_TO_TARGET_TYPE(name))(____HOOK_MAKE_ARGS(__VA_ARGS__)); \
  spec ____HOOK_DECLARE(void, name, NULLABLE(void*), ignored, NONNULLABLE(struct hook_call_info*), ci __VA_OPT__(,) __VA_ARGS__); \
  spec enum hook_action ____HOOK_TO_HANDLER(name)(NONNULLABLE(struct hook_call_info*) ci, va_list args) { \
    name(NULL, ci __VA_OPT__(,) MACRO_FOR_EACH_STRIDE2(____HOOK_PROCESS_ARGUMENT, ____HOOK_TARGET_IGNORE, __VA_ARGS__)); \
    return ci->action; \
  } \
  spec ____HOOK_DECLARE(void, name, NULLABLE(void*), , [[maybe_unused]] NONNULLABLE(struct hook_call_info*), ci __VA_OPT__(,) __VA_ARGS__)
